...
## 구현할 기능 목록
1. 자동차 이름을 입력받음. (쉼표로 구분, 5자 이하만 가능)
2. 시도할 횟수를 입력받음.
3. 사용자가 잘못된 값 입력시 에러 발생.
4. 0~9 사이의 무작위 값 생성, 4 이상이면 한 칸 전진함.
5. 각 차수별 실행 결과를 출력함. 
6. 단독 or 공동 우승자 안내 문구를 출력함.

## 느낀점
- 이번에는 기능별로 커밋을 하는 것에 신경을 썼는데, 직접 해보며 어떻게 하는건지 감을 잡을 수 있어 좋았습니다.
클래스를 2개로 나누다보니, 우왕좌왕하면서 헷갈렸지만 붙잡고 작성하려고 노력했습니다. 
하지만 아직 이름에 대한 예외처리 부분에서 통과하지 못해 미완성인데 왜 통과하지 못하는건지 궁금합니다!
---
...
- JDK에서 기본 제공하는 Random, Scanner API 대신 `mallang.missionutils`에서 제공하는 `Randoms`, `Console` API를 활용해 구현해야 한다.
